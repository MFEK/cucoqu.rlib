...
use nalgebra::{Matrix, Const, ArrayStorage, vector};
...
impl PointLike for Point3 {
    fn x(&self) -> IntegerOrFloat { Float(self.x) }
    fn y(&self) -> IntegerOrFloat { Float(self.y) }
    fn set_x(&mut self, x: IntegerOrFloat) { self.x = x.into(); }
    fn set_y(&mut self, y: IntegerOrFloat) { self.y = y.into(); }
}
...
impl Point3Like for Point3 {
    fn z(&self) -> IntegerOrFloat { Float(self.z) }
    fn set_z(&mut self, z: IntegerOrFloat) { self.z = z.into(); }
}
...
impl Point3 {
    ...
    pub fn new(x: impl Into<f32>, y: impl Into<f32>, z: impl Into<f32>) -> Self { Self {x: x.into(), y: y.into(), z: z.into()} }
